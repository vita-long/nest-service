...
export class User {
   ...
   * 用于账号验证和通知，唯一，可选
   */
  @Column({ name: 'email', unique: true, nullable: true, comment: '电子邮箱' })
  email?: string;
   ...
   * 存储加密后的密码，不能为空
   */
  @Column({ name: 'password', nullable: false, comment: '密码' })
  password: string;
   ...
   * 用于账号验证和通知，唯一，可选
   */
  @Column({ name: 'phone', unique: true, nullable: true, comment: '手机号码' })
  phone?: string;
   ...
   */
  @Column({ name: 'role', default: 'user', comment: '用户角色' })
  role: string;
   ...
   */
  @Column({ name: 'bio', nullable: true, length: 500, comment: '个人简介' })
  bio?: string;
   ...
   */
  @Column({ name: 'deleted_at', nullable: true, comment: '删除时间' })
  deletedAt?: Date;
}