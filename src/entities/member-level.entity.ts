...
export class MemberLevel {
   ...
   * 该等级会员的订阅价格，种子会员为0
   */
  @Column({ name: 'subscription_price', type: 'decimal', precision: 10, scale: 2, default: 0, comment: '订阅价格' })
   ...
   */
  @Column({ name: 'validity_period', type: 'int', default: 0, comment: '有效期(月)' })
   ...
   * 每月可领取的免运费券数量
   */
  @Column({ name: 'free_shipping_tickets', type: 'int', default: 0, comment: '每月免运费券数量' })
   ...
   * 是否享受全年无限次免运费
   */
  @Column({ name: 'unlimited_free_shipping', default: false, comment: '全年无限次免运费' })
   ...
   * 每月可享受的免费花束升级次数
   */
  @Column({ name: 'free_bouquet_upgrades', type: 'int', default: 0, comment: '每月免费花束升级次数' })
   ...
   * 是否在重大节日赠送神秘高阶花礼
   */
  @Column({ name: 'holiday_gifts', default: false, comment: '重大节日赠送神秘高阶花礼' })
   ...
   * 会员等级的详细描述
   */
  @Column({ name: 'description', type: 'text', nullable: true, comment: '等级描述' })
  description?: string;
