...
export class HealthController {
  ...
  async getRedisCacheInfo(@Query('pattern') pattern?: string) {
    try {
      ...
      if (isConnected) {
        ...
        const cacheDetails = await this.redisCacheService.getAllCacheInfo(
          pattern || '*',
        );
  ...
  async deleteRedisCache(
    ...
    try {
      ...
      for (const key of keysToDelete) {
        ...
        const keyExists = usePrefix
          ? await this.redisCacheService.exists(key, currentPrefix)
          : true;
        ...
        const deleteResult = await this.redisCacheService.delete(
          key,
          currentPrefix,
        );
  ...
  async deleteAllRedisCache(@Query('pattern') pattern?: string) {
