  ...
  async create(createOrderDto: CreateOrderDto): Promise<Order> {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const productItem of products) {
        ...
        // 使用数据库中查询的产品价格，确保价格的准确性和完整性
        const currentPrice = product.price || product.basePrice || 0;

        validatedProducts.push({
          product,
          quantity: productItem.quantity,
          price: currentPrice, // 使用当前价格（快照）
          subtotalAmount: currentPrice * productItem.quantity,
        });

        goodsAmount += currentPrice * productItem.quantity;
      }

      // 3. 优惠券验证和优惠金额计算
      let discountAmount = 0;
      const validatedCoupons: Array<{
        couponRecord: CouponReceiveRecord;
        coupon: Coupon;
      ...
      if (couponIds && couponIds.length > 0) {
        for (const couponId of couponIds) {
          // 查询用户领取的优惠券记录
          const couponRecord = await manager
            .getRepository(CouponReceiveRecord)
            .findOne({
              where: {
                id: couponId,
                userId: userId,
      ...
      for (const couponInfo of validatedCoupons) {
        await manager
          .getRepository(CouponReceiveRecord)
          .update(couponInfo.couponRecord.id, {
      ...
      // 9. 库存预扣 - 将事务管理器传递给updateStock方法，确保在同一个事务中执行
